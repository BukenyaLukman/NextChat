...
def friend_list_view(request, *args, **kwargs):
	...
	if user.is_authenticated:
		...
		if user_id:
			...
			# must be friends to view friends list
			if user != this_user:
...
def accept_friend_request(request, *args, **kwargs):
	...
	if request.method == 'GET' and user.is_authenticated:
		...
		if friend_request_id:
			...
			payload['response'] = "Unable to acept that friend request"
	else:
		payload['response'] = "You must authenticated to accept friend request"
	return HttpResponse(json.dumps(payload),content_type="application/json")
	...
	if request.method == "POST" and user.is_authenticated:
		...
		if user_id:
			try:
				...
				payload['response'] = "Successfully remvoed that friend"
...
def cancel_friend_request(request, *args, **kwargs):
	...
	if request.method == "POST" and user.is_authenticated:
		...
		if user_id:
			...
			# there should only be ever a single active friend request at a time
